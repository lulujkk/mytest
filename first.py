...
<style>
    ...
    /* 核心新增：强制X轴标签横向显示（取消旋转、居中对齐） */
    /* 折线图/面积图/柱状图的X轴标签样式 */
    .st-chart svg g[class*="xtick"] text {
        writing-mode: horizontal-tb !important; /* 横向排版 */
        transform: rotate(0deg) !important; /* 取消旋转 */
        text-anchor: middle !important; /* 居中对齐 */
        font-size: 14px !important; /* 字体大小适配 */
    }
    /* 增加X轴区域高度，避免标签重叠 */
    .st-chart svg {
...
# --------------------------
# 1. 数据准备（优化：经纬度精度提升，地址更规范，后续转换为原生类型）
# --------------------------
...
df_category_booking = convert_numpy_to_python(df_category_booking)
...
df_review = convert_numpy_to_python(df_review)

# （5）关键指标计算（优化：新增平均预订量、价格涨幅，转换为原生类型）
max_rating = float(df_shops["评分"].max())
min_rating = float(df_shops["评分"].min())
avg_consumption = round(float(df_shops["小包厢小时消费(元)"].mean()), 1)
total_bookings = int(df_shops["月均预订量(次)"].sum())
avg_bookings = round(float(df_shops["月均预订量(次)"].mean()), 0)
# 价格涨幅（最高价格/最低价格-1）
price_increase = round((float(df_shops["小包厢小时消费(元)"].max()) / float(df_shops["小包厢小时消费(元)"].min()) - 1) * 100, 1)
...
# 消费区间筛选
min_price = int(df_shops["小包厢小时消费(元)"].min())
max_price = int(df_shops["小包厢小时消费(元)"].max())
price_range = st.sidebar.slider(
    "选择小包厢小时消费区间(元)",
    min_value=min_price,
    max_value=max_price,
    value=(min_price, max_price),  # 默认选中实际价格范围
    step=5,
...
# 核心修复：筛选后再次转换为原生类型，确保无残留NumPy类型
df_shops_filtered = convert_numpy_to_python(df_shops_filtered)
...
with col4:
    st.metric(
        ...
        delta=f"{avg_bookings - int(df_shops['月均预订量(次)'].min()):.0f} 次（高于最低值）"
    )

st.markdown("---")

# 第二行：店铺信息 + 价格走势折线图（核心修改：列宽从[1,2]→[1,3]，增大图表横向空间）
col1, col2 = st.columns([1, 3])  # 图表列宽度增加，给X轴更多空间
...
# 第三行：KTV类型预订量柱状图 + 用户评价面积图（优化：面积图高度同步调整）
...
# 第四行：KTV店铺位置地图（核心修复：处理空数据+size为整数+移除可能的兼容问题）
st.subheader("南宁KTV店铺位置分布")
# 修复1：处理空数据，避免无数据时报错
if len(df_shops_filtered) == 0:
    st.warning("⚠️ 暂无符合条件的KTV店铺数据，无法显示地图！")
else:
    # 修复2：size参数转为整数（避免浮点数序列化问题）
    map_size = (df_shops_filtered["月均预订量(次)"] // 5).astype(int)  # 用整除代替除法，直接得到整数
    # 修复3：调用st.map时，确保所有参数为原生类型，简化参数减少兼容问题
    st.map(
        df_shops_filtered,
        latitude="纬度",
        longitude="经度",
